...
2. 填入服务器相关信息：
   ...
   - `服务器地址(IP/域名)`: Minecraft 服务器直连地址（如 `dx.sssysy.top` 或 `127.0.0.1:25565`），用于 `mc查看` 命令查询在线状态、版本、简介与在线玩家列表
...
| 配置项 | 说明 |
| :--- | :--- |
| **群聊显示服务器外显名** | 开启后 MC -> 群聊消息形如：`[外显名] <sssysy> 你好` |
...
| `mc查看定时公告` | 查看所有定时公告的配置、规则与下次运行时间 | 管理员 |
