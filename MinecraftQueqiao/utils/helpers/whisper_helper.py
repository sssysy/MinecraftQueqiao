...
from .component import parse_text_or_json_component
from .user_select import extract_at_user_ids


async def parse_whisper_args(
    ev: Event,
...
async def get_target_servers(ev: Event) -> Tuple[List[MCQQServer], Optional[str]]:
    """获取要发送私聊的目标服务器列表。"""
...
async def handle_whisper(bot: Bot, ev: Event) -> None:
    """向 Minecraft 玩家发送私聊消息（通过 tellraw）。
    用法：
      mc私聊 <@用户 / QQ号 / 游戏ID> <私聊内容>
    """
    player_name, content, err = await parse_whisper_args(ev)
    if err:
        await bot.send(err)
        return
    ...
    servers, srv_err = await get_target_servers(ev)
    if srv_err:
